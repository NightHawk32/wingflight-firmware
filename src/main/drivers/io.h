...
// same bit layout as STM32F4: mode(0-1) | drive_strength(2-3) | out_type(4) | pull(5-6)
#define IO_CONFIG(mode, speed, otype, pupd) ((mode) | ((speed) << 2) | ((otype) << 4) | ((pupd) << 5))

#define IOCFG_OUT_PP         IO_CONFIG(GPIO_MODE_OUTPUT, GPIO_DRIVE_STRENGTH_MODERATE, GPIO_OUTPUT_PUSH_PULL,  GPIO_PULL_NONE)
...
#define IOCFG_AF_OD_UP       IO_CONFIG(GPIO_MODE_MUX,    GPIO_DRIVE_STRENGTH_MODERATE, GPIO_OUTPUT_OPEN_DRAIN, GPIO_PULL_UP)
...
#define IOCFG_IPU_25         IO_CONFIG(GPIO_MODE_INPUT,  GPIO_DRIVE_STRENGTH_STRONGER, GPIO_OUTPUT_PUSH_PULL,  GPIO_PULL_UP)
