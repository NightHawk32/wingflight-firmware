...
// PICO (RP2350/RP2354) does not use the STM32 lettered-GPIO-port / TARGET_IO_PORTx
// scheme below: pico-sdk addresses all GPIOs via one flat 0..47 numbering, so the
// whole port-lettered block is replaced with a single wide virtual "port A" of up
// to 64 pins (DEFIO_PORT_PINS override in common/platform.h), ported from
// betaflight/src/platform/PICO/io_def_generated.h and extended to RP2354A/RP2354B
// (same pinout as RP2350A/RP2350B respectively, see RP2350_UNIFIED/target.h).
#if !defined(PICO)
...
// PICO (RP2350/RP2354): single flat virtual port ("port A") covering GPIO0..47,
// ported from betaflight/src/platform/PICO/io_def_generated.h. RP2354A/RP2354B
// are derived variants with the same pinout as RP2350A/RP2350B (on-die flash
// instead of external QSPI only), so they share the same pin counts/tags.
...
// DEFIO_PORT_USED_COUNT / LIST / OFFSET_LIST are not used by the PICO drivers
// (single flat port, see drivers/io_pico.c), kept only so any generic code that
