...
/*
 * AT32F435/437 USB CDC-ACM (virtual COM port) driver, adapted from
 * betaflight/src/platform/AT32/serial_usb_vcp_at32f4.c to Wingflight's older
 * BF4.3-era conventions.
 *
 * Unlike STM32F4/F7/H7/G4 (where the shared drivers/serial_usb_vcp.c file provides
 * usbVcpOpen()/the serial vTable and only calls out to a handful of small per-MCU
 * CDC_* helper functions), Artery's usbd_core middleware is architecturally different
 * enough from ST's USB device library that this file is a FULL, self-contained
 * replacement -- it implements its own usbVcpOpen()/usbVcpGetBaudRate()/vTable
 * directly, exactly mirroring the upstream AT32 reference file. Because of this,
 * drivers/serial_usb_vcp.c (which would otherwise duplicate usbVcpOpen()) is excluded
 * from the AT32 build via MCU_EXCLUDES in make/mcu/AT32F4.mk.
 */
...
// Called by the vendored cdc_class.c when the host issues a SET_LINE_CODING request
// (i.e. changes the virtual COM port baud rate). Not called anywhere in Artery's own
// SDK/examples, only ever declared `extern` and invoked from cdc_class.c -- this is the
// definition that satisfies that extern.
...
// Required by usb_conf.h (called by the vendored usb_drivers middleware); implemented
// using Wingflight's own delay primitives rather than a bespoke busy-loop.
