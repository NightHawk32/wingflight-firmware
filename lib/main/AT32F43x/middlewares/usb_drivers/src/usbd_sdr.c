...
/* Wingflight: vendor SDK bug fix -- this TU uses NULL (from usbd_get_descriptor())
 * without including <stddef.h>, and no header in its include chain (usbd_sdr.h ->
 * usb_conf.h/usb_core.h -> the AT32F43x CMSIS device header) pulls it in transitively. */
#include <stddef.h>
