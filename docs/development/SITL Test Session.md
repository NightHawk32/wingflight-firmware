...
| 3 | Bare SITL start | Run `wingflight_SITL.elf` alone: first-boot eeprom write, UART port binds | User | |
| 4 | Configurator | Connect over `tcp://127.0.0.1:5761`, browse tabs, save/"reboot" | User | |
| 5 | Joystick RC | `sitl-joystick-rc.py` GUI, channel mapping, Receiver tab shows sticks | User | |
| 6 | Bridge + joystick flight | Launcher `-Trim -Joystick`, arm, fly by bridge status line | User | |
...
|---|---|---|---|---|
| I-1 | 2 | Medium | Initial latitude is set as geocentric (`ic/lat-gc-deg` in `jsbsim_bridge.py`) but GPS and FlightGear report geodetic. The aircraft starts at geodetic 37.80 N instead of KSFO's 37.61 N, about 21 km north. The `-Mode gps` check still passes because it only checks the fix is "near" the IC. The launcher also pre-positions FlightGear at 37.6136, so scenery preloads at the wrong spot. | Open |
