...
| I-8 | 7 | Low | Controls fly the right way, but in FlightGear the rudder and right aileron moved the wrong way. The model streamed raw JSBSim signs, while FlightGear models expect FGJSBsim's convention: right aileron with the same sign as the left, rudder negated. Checked against the Edge 540's hinge axes and animation factors. **Fix:** flipped the two display-only `*-pos-norm` outputs in the FCS. | Fixed |
| I-9 | 7 | High | The airborne start handed a trimmed glider to a disarmed FC at zero throttle, so it crashed right away. The crash then showed that a wingtip strike integrated the state to NaN. Two causes: stiff structure contacts on a 0.45 kg·m² roll inertia, and JSBSim's friction solver emitting ~1e5 lbf gear forces when several contacts hit at once. **Fix:** ground start on runway 28R by default for the 3D model (`--start ground`, taildragger resting at 7.9°). Softer, lower-friction structure contacts. 4 physics sub-steps per 120 Hz cycle (480 Hz). The bridge treats rates above 50 rad/s or speeds above 150 m/s as a blow-up and resets. A crash resting nosed over or inverted is reset after 2 s. Verified offline: rest, take-off (lift-off after 0.8 s at 13 m/s), landing (0.9 m/s sink) all without false resets; 80 random impacts produce no NaN reaching SITL. `sitl-rc-check.ps1` forces `--start air`, and jsbsim/throttle/gps pass. | Fixed |
| I-10 | 7 | Low | Needed a fixed camera instead of the chase camera. **Fix:** FlightGear starts in Tower view (`-FgView 3`), with the "tower" moved to an RC pilot position 30 m beside the runway, 60 m past the threshold, eye height 1.7 m (`-PilotAlongM` / `-PilotSideM`). The view's near plane is lowered from 10 m to 0.5 m. | Fixed, visual check pending |
